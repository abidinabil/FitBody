...
>
    ...
    >
      <v-btn-toggle
        ...
        group
      >
        <v-btn value="" @click.prevent="getProduitByCategorie(this.Proteine1)">
          All Produit
        </v-btn>
        <v-btn value="" @click.prevent="getProduitBySousCategorie(this.Proteine)">
          Proteine
        </v-btn>
        <v-btn value="" @click.prevent="getProduitBySousCategorie(this.CEREALES)" >
          CÉRÉALES
        </v-btn>
         <v-btn value="" @click.prevent="getProduitBySousCategorie(this.MICRONUTRIMENTS)" >
          MICRONUTRIMENTS
        </v-btn>
    ...
    <v-row>
      <v-col cols="12" xs="12" sm="6" md="4" lg="3" v-for="proteine in ProduitProteine" :key="proteine.name">
          <v-card 	>
        <div class="ui-card">
            <v-img   :aspect-ratio="1"   v-bind:src="'../image/boutique/' + proteine.image" 
                   cover    > </v-img>
        ...
        <v-spacer></v-spacer>
        <p style="color: black; font-size: 20px;">{{proteine.price}}</p>   
      </v-toolbar>
         <p class="mx-2">{{proteine.categorie}}</p>
      <p class="mx-2">{{proteine.sous_categorie}}</p>
        </v-col>
        </v-row>
  ...
  components: { NavbarView },
   data(){
    ...
    methods: {
       ...
       axios.get('http://localhost:8000/api/auth/getProduitProteine/'+$sous_categorie)
        .then (res => {
         console.log(res.data);
         this.ProduitProteine = res.data;
       }).catch(
